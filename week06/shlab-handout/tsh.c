...
void eval(char *cmdline)
{
	...
	if (!builtin_cmd(argv))
	{
		...
		addjob(jobs, pid, (bg == 1 ? BG : FG), cmdline); // Adding Job to JobList Array
		if (!bg)
		{
			waitpid(pid, NULL, 0);
			deletejob(jobs, pid);
		...
		{
			printf("(%d) (%d) %s", pid2jid(pid), pid, jobs[pid2jid(pid) - 1].cmdline);
		}
	}
	return;
}
...
{
	char *cmd = argv[0];
	if (cmd == '\0') /* new line exception */
		return 1;
	if (!strcmp(cmd, "quit") || !strcmp(cmd, "exit")) /* quit command */
	{
		fflush(stdout);
		fflush(stderr);
		exit(0);
