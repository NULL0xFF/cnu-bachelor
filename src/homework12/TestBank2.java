...
public class TestBank2 {
  ...
  public static void main(String[] args) {
    ...
    for (int cus_index = 1; cus_index <= bank.getNumberOfCustomers(); cus_index++) {
      ...
      for (int acc_index = 1; acc_index <= bank.getCustomer(cus_index).getNumOfAccounts();
          acc_index++) {
    ...
    for (int cus_index = 1; cus_index <= bank.getNumberOfCustomers(); cus_index++) {
      ...
      for (int acc_index = 1; acc_index <= bank.getCustomer(cus_index).getNumOfAccounts();
        ...
        if (bank.getCustomer(cus_index).getAccount(acc_index) instanceof SavingsAccount sa) {
          sa.updateBalance(10);
          System.out.print("계좌 종류: SavingsAccount, ");
        }
