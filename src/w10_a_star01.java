...
public class w10_a_star01 {
	...
	public static void main(String[] args) {
		...
		while (!OpenList.isEmpty()) {
			...
			if (flag)
				CloseList.add(nodePointer);
			OpenList.addAll(adjacentNodeList(graph, nodePointer, E));
		}
		...
		while (true) {
			...
			while (true) {
				if ((CloseList.get(CloseList.size() - 1).id.x == curPos.x)
						&& (CloseList.get(CloseList.size() - 1).id.y == curPos.y)) {
					curNode = CloseList.get(CloseList.size() - 1);
					break;
				}
				CloseList.remove(CloseList.get(CloseList.size() - 1));
			}
		}
		/* Path Complete */
		////////////////////////////////
