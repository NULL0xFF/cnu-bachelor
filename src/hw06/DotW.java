...
public class DotW {
	// Global Declaration
	private static int[] date = { 0, 0, 0 }; // 입력된 날짜의 (연, 월, 일)Array
	...
	private static boolean input_test() {
		...
		case 2:
			// 2월일 경우, 윤년인지 확인
			if (leapyear_boolean(date[2]) == true) {
	...
	private static void input_days() {
		...
		for (int n = date[0] - 1900; n > 0; n--) {
			if (leapyear_boolean(n + 1899) == true) {
				days = days + 366;
			} else {
				days = days + 365;
			}
		...
		for (int n = date[1] - 1; n > 0; n--) {
			...
			case 12:
				...
				break;
			case 2:
				if (leapyear_boolean(date[0]) == true) {
					days = days + 29;
				} else {
					days = days + 28;
				}
	...
	public static void main(String[] args) {
		...
		} else if ((date[0] < 1900) || (date[0] == 1900 && date[1] <= 2) || (date[0] == 2100 && date[1] >= 3)
				|| (date[0] > 2100)) {
